...
public class PushupsDisplay extends android.app.Fragment {
    DatabaseHelper db;
    LineChart lineChart;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        final View view = inflater.inflate(R.layout.pushupsdisplay,null);
        db = new DatabaseHelper(getActivity());
        graphing = new LineGraphSeries<DataPoint>();
        GraphView graph = (GraphView)view.findViewById(R.id.graphPushups);
        ...
        if (num!=0) {
            while (cursor.moveToNext()) {
                int i = 0;
                String test = cursor.getString(cursor.getColumnIndex("pushScore"));
